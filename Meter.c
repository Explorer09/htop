...
static inline GraphData* GraphData_new(size_t nValues, size_t nItems, bool isPercentGraph) {
...
static inline int GraphData_getColor(GraphData* this, int vIndex, int h, int scaleExp) {
...
static void BarMeterMode_draw(Meter* this, int x, int y, int w) {
   ...
   for (int i = 0; i < items; i++) {
      ...
      if (this->values[i] * itemWidth < full) {
         blockSizes[i] = 2;
         countB++;
   ...
   // additional space.
   double widthB = 0.0;
   if ((double)countB * (full - totalA) < totalB * (w - countA)) {
      widthB = (totalB * (w - countA) / (full - totalA)) - countB;
   }
   double widthC = (double)(w - countA - countB) - widthB;
   double totalC = full - totalA - totalB;
   // Pass 3/3: Plot.
   int offset = 0;
   ...
   for (int i = 0; i < items; i++) {
      ...
      if (blockSizes[i] == 1) {
         ...
         plot = plotted + 1.0;
         if (widthB > 0.0) {
            plot += widthB * (this->values[i] * w - full) /
                    (totalB * w - full * countB);
         }
      } else {
...
static void GraphMeterMode_printScale(unsigned int scaleExp) {
   static const char* divisors = "842";
...
static void GraphMeterMode_draw(Meter* this, int x, int y, int w) {
   ...
   if (!this->drawData) {
      this->drawData = (void*) GraphData_new(nValues, Meter_getMaxItems(this),
                                             isPercentGraph);
   }
   ...
   if (!timercmp(&now, &(data->time), <)) {
      ...
      data->values[nValues - 1] = stack2[items];
      double scale;
      if (isPercentGraph) {
         data->values[nValues - 1] /= this->full;
      ...
      for (int step = 1; ; step <<= 1) {
         ...
         for (int h = 0; ; h += step) {
            ...
            data->colors[(nValues - 2) * data->colorRowSize + offset] = color;
            if (data->drawOffset > 0) {
               data->colors[(nValues - 1) * data->colorRowSize + offset] = color2;
            }
         }
